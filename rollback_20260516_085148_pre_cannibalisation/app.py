...
def _pnl_compute(state='all', cluster='all', cohort='all', venue_q='all', apply_guards=True):
    ...
    for r in results:
        ...
        # Resolved daily sales (with scenarios + guardrails)
        sales_resolved, _override, _gnote = _resolve_venue_sales(r, apply_guardrails=apply_guards)
...
def pnl_view():
    ...
    venue_q = request.args.get('venue', 'all')
    apply_guards = request.args.get('guardrails', 'on').lower() in ('on', '1', 'true', 'yes')
    out = _pnl_compute(state, cluster, cohort, venue_q, apply_guards)
    if out is None:
        return jsonify({'error': 'No forecast results. Run /generate first.'}), 400
    return jsonify(out)
